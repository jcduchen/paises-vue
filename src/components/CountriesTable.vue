<template>
  <div>
    <input type="text" class="form-control mb-3" placeholder="Digite el nombre del país para buscar" :value="search" @input="e => search = e.target.value">
      ...
      <transition-group name="countries-list" tag="tbody">
        <tr v-for="(country, index) in pagedCountries" v-bind:key="'pais_' + index">
          ...
          <td class="align-middle text-right">
            <router-link :to="'/detalhes/' + country.alpha2Code" tag="button" class="btn btn-sm btn-primary">
              <div class="d-flex flex-row align-items-center">
                <i class="fas fa-edit "></i> <span class="d-none d-sm-block ml-2">editar</span>
              </div>
            </router-link>
          </td>
          ...
          <td class="align-middle text-right">
            <router-link :to="'/detalhes/' + country.alpha2Code" tag="button" class="btn btn-sm btn-primary">
              <div class="d-flex flex-row align-items-center">
                <i class="fas fa-eraser"></i> <span class="d-none d-sm-block ml-2">Eliminar</span>
              </div>
            </router-link>
          </td>
          ...
          <td class="align-middle text-right">
            <router-link :to="'/detalhes/' + country.alpha2Code" tag="button" class="btn btn-sm btn-primary">
              <div class="d-flex flex-row align-items-center">
                <i class="fas fa-eye"></i> <span class="d-none d-sm-block ml-2">informacion</span>
              </div>
            </router-link>
          </td>
        </tr>
    ...
    <nav aria-label="Navegaçion por páginas" v-if="pages > 1">
